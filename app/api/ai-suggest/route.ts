...
Сегодня: ${todayStr}, т.е. ${todayISO} (точка отсчёта для относительных сроков вроде "до среды").
...
2. Если срок явно указан (в тексте или отдельно) — переведи его в конкретную дату YYYY-MM-DD, отталкиваясь от сегодняшней даты. Если срок нигде не указан — deadline = "${todayISO}" (сегодня).
