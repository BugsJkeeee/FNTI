...
Список проектов (сопоставляй по шифру даже с опечатками/другим регистром — "иерархия"→"Иерархия", "гсу"→"ГСУ-500", "хит"→"ХИТ-ВА"; по номеру проекта, если назван числом; по исполнителю — ТОЛЬКО если он однозначно называет один проект, если у нескольких проектов совпадает исполнитель и по тексту не ясно, какой именно — верни project_id: null и объясни в reason, что нужно уточнить по шифру/номеру):
...
5. project_intent = "checklist_done": отметить шаг чек-листа выполненным или снять отметку.
   ...
   - ИСКЛЮЧЕНИЕ: если в формулировке явно упомянуто слово "требование" (в любом падеже) — это НИКОГДА не про шаг fin_9, а про конкретную запись требования о возврате: используй интент "claim_execution_date" (пункт 8), а не "checklist_done" с template_key "fin_9", даже если по смыслу фраза похожа на "Исполнено требование о возврате".
...
10. project_intent = "summary": просьба дать сводку/статус/что происходит по одному или нескольким проектам (триггеры: "сводка по", "статус проекта", "что там с", "как дела у", "расскажи про проект"). В отличие от остальных интентов здесь может быть НЕСКОЛЬКО проектов сразу (например "сводка по проектам Удар и Доцент") — верни ВСЕ найденные id массивом в project_ids (каждый по тем же правилам сопоставления, что выше), а project_id оставь null. Если хотя бы один из названных проектов не удалось однозначно определить — всё равно верни те, что определились, и объясни в reason, какой не нашёлся.
  ...
  async function callOpenRouter(model: string, signal: AbortSignal) {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.2,
        // Схема ответа выросла (домены "задача" + "проект" в одном JSON, 17 полей) — часть бесплатных
        // моделей "думает вслух" перед ответом (поле reasoning), при 700 токенов иногда обрезало
        // JSON на середине (finish_reason: "length"). С запасом на рассуждения + сам JSON.
        max_tokens: 1500,
      }),
    })
